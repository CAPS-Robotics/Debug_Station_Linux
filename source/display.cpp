...
{
    glLoadIdentity();
    glViewport(0, (winHeight/6), winWidth/2, (winHeight/6) * 5);
...
{
    glLoadIdentity();
    glViewport(winWidth/2, winHeight/6, winWidth/2, (winHeight/6) * 5);
...
void graph_controlled_values()
{
    glViewport(0, 0, winWidth, (winHeight/6));

    glTranslatef(-6, -2.9, -5);

    for(int i = 0; i < 6; i++)
    {
        if(i == control_value_select)
        {
            glColor3f(0.f, 0.f, 0.f);
        } 
        else 
        ...
        glBegin(GL_POLYGON);
            glVertex2d(0 + (i*3), 0);
            glVertex2d(1 + (i*3), 0);
            glVertex2d(1 + (i*3), 1 + control_values[i]);  
            glVertex2d(0 + (i*3), 1 + control_values[i]);
        glEnd();
    }
}
